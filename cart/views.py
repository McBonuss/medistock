...
def _parse_quantity(raw_value, fallback=1):
    try:
        qty = int(raw_value)
...
def add_to_cart(request, product_pk):
    ...
    cart = get_or_create_cart(request.user)
    qty = _parse_quantity(request.POST.get('quantity', 1), fallback=1)
...
def update_item(request, item_pk):
    ...
    qty = _parse_quantity(request.POST.get('quantity', item.quantity), fallback=item.quantity)
    item.quantity = qty
    item.save()
